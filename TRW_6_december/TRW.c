...
void Banal_Extrapolation(struct parr *px, int i, int j);
/**************************SeLf-CoNsIsTeNcE****************************/
...
void initialarray(struct pmct z,struct parr *px){
	...
	for(i=0;i<z.Nt2;i++){
		...
		px->C[i+1][i+1]=1;
		px->R[i+1][i+1]=0;
		px->f1[i+1][i+1] = f1(px->C[i+1][i+1],z);
		px->f2R[i+1][i+1] = f2(px->C[i+1][0],z)*px->R[i+1][0];

		for(j=i;j>=0;j--){

			if2RC = grid_If2RC(*px,i,j); //If2RC(z,*px,i,j); //
			if2RR = grid_If2RR(*px,i,j); //If2RR(z,*px,i,j); //
			if1R = grid_If1R(*px,i,j); //If1R(z,*px,i,j); //
		...
		if2RC = grid_If2RC(*px,i+1,i+1); //If2RC(z,*px,i+1,i+1); //
		if1R = grid_If1R(*px,i+1,i+1); //If1R(z,*px,i+1,i+1); //
...
int step(int i,struct pmct z,struct parr *px,struct psys w){
	...
	for(j=i;j>0;j--) { Banal_Extrapolation(px,i,j); Mirroring(px,i,j); }
	...
	//****** PART ----> j<i-1
		while(j>=0 && j<i-2){
			...
			D = D1/px->dt + px->mu[i] - I1*px->df2Rv[i][i-1]*px->dt;
...
void Banal_Extrapolation(struct parr *px, int i, int j) {
	...
	if (j>0) {
		...
		px->R[i][j]= px->R[i-1][j-1];
		px->f1[i][j]= px->f1[i-1][j-1];
		px->f2R[i][j]= px->f2R[i-1][j-1];
		px->dCh[i][j]= px->dCh[i-1][j-1];
		px->dRh[i][j]= px->dRh[i-1][j-1];
		px->df1h[i][j]= px->df1h[i-1][j-1];
		px->df2Rh[i][j]= px->df2Rh[i-1][j-1];
		px->dCv[i][j]= px->dCv[i-1][j-1];
		px->dRv[i][j]= px->dRv[i-1][j-1];
		px->df1v[i][j]= px->df1v[i-1][j-1];
		px->df2Rv[i][j]= px->df2Rv[i-1][j-1];
	} else {
		px->C[i][j]= px->C[i-1][j];
		px->R[i][j]= px->R[i-1][j];
		px->f1[i][j]= px->f1[i-1][j];
		px->f2R[i][j]= px->f2R[i-1][j];
		px->dCh[i][j]= px->dCh[i-1][j];
		px->dRh[i][j]= px->dRh[i-1][j];
		px->df1h[i][j]= px->df1h[i-1][j];
		px->df2Rh[i][j]= px->df2Rh[i-1][j];
		px->dCv[i][j]= px->dCv[i-1][j];
		px->dRv[i][j]= px->dRv[i-1][j];
		px->df1v[i][j]= px->df1v[i-1][j];
		px->df2Rv[i][j]= px->df2Rv[i-1][j];
	}
}
...
double SC2(double *gC, double *gR, double D, double mu, struct pmct z, struct parr x, int i, int j){
	...
	gC[j]+= x.df2Rv[i][i-1]*x.dt*(I3*x.C[i-2][j]*+I2*x.C[i-1][j]);
	gC[j]/= D;
	gC[j]-= x.C[i][j];
	gR[j] = -D3/x.dt*x.R[i-2][j]-D2/x.dt*x.R[i-1][j]+if2RR;
	gR[j]+= x.df2Rv[i][i-1]*x.dt*(I3*x.R[i-2][j]*+I2*x.R[i-1][j]);
	gR[j]/= D;
	gR[j]-= x.R[i][j];
	

 	printf("%d %d %f %e %e\r",j,i,x.C[i][j],gC[j],gR[j]); fflush(stdout); getchar();
...
void parameters_initialization(struct pmct *pz,struct parr *px, struct psys *pw, char *argv[]) {
	...
	pz->den  = 1 << 5;			// Nt/Nc
	pz->Nc = 1 << 2;
	pz->itr = 20;				// number of cycle.

	pz->t0 = 1.0E-2;				// Initial time window
	pz->Cmin = 1.0E-12;			// Minimal value accepted before setting C identically equal to 0.
	pz->eps= 1.0E-10; 			// Accepted error distance between the solution and the discrete equation ->
	pz->rpt= 1000;				// 		in the iteration procedure with z.rpt the maximum number of iterations

	pz->alpha = 0.1;				// Coefficient of self-consistence iteration
...
void write_C_0(struct parr x, struct psys w, int ini, int ifi){
	...
	for(i=ini;i<=ifi;i++) {
		fprintf(fout,"%1.5e\t%1.5e\t%1.5e\t%1.5e\t%1.5e\t%1.5e\t%1.5e\t%1.5e\t%1.5e\n",
			(double)i*x.dt,x.C[i][0],x.dCv[i][0],x.dCh[i][0],x.R[i][0],x.dRv[i][0],x.dRh[i][0],x.mu[i],x.E[i]);
  	}
	fclose(fout);
}
...
void write_C(struct pmct z, struct parr x, struct psys w, int j){
	...
	for(i=1;i<=z.Nt;i++) {
		fprintf(fout,"%1.5e\t%1.5e\t%1.5e\t%1.5e\t%1.5e\t%1.5e\t%1.5e\n",(double)(i-j)*x.dt,x.C[i][j],x.dCv[i][j],x.dCh[i][j],x.R[i][j],x.dRv[i][j],x.dRh[i][j]);
  	}
	fclose(fout);
}
